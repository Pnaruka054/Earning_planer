...
const heroEdit = async (req, res) => {
    try {
        const text = req.body.texts[0]; // Use only the first text
        ...
        if (existingData) {
            if (req.file) {
                ...
                existingData.images = req.file.path; // Assuming req.file.path contains the new image URL
            }
